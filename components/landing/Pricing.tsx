...
const PricingCard = ({ title, price, features, recommended, delay }: { title: string, price: string, features: string[], recommended?: boolean, delay: number }) => (
    <motion.div 
        ...
        className={`p-8 rounded-3xl border flex flex-col h-full transition-all duration-300 hover:shadow-2xl relative
            ${recommended 
                ? 'border-teal-500 bg-gray-800/80 shadow-teal-500/10 scale-105 z-10' 
    ...
    >
        ...
        <div className="mb-8">
            <h3 className={`text-lg font-semibold mb-2 ${recommended ? 'text-teal-400' : 'text-gray-300'}`}>{title}</h3>
            <div className="flex items-baseline">
                <span className="text-4xl font-extrabold text-white tracking-tight">{price}</span>
                {price !== 'Free' && <span className="text-gray-500 ml-2 font-medium">/month</span>}
            </div>
        </div>
        ...
        <ul className="space-y-4 mb-8 flex-1">
            {features.map((feature, i) => (
                <li key={i} className="flex items-start">
                    <CheckIcon className={`w-5 h-5 mr-3 flex-shrink-0 mt-0.5 ${recommended ? 'text-teal-400' : 'text-gray-500'}`} />
...
const Pricing: React.FC = () => {
    return (
        <section id="pricing" className="py-24 md:py-32 bg-gray-900 relative">
             <div className="container mx-auto px-4 sm:px-6 lg:px-8">
                ...
                >
                    ...
                    <h2 className="text-4xl md:text-5xl font-extrabold text-white mb-6 tracking-tight">Simple, transparent pricing</h2>
                    <p className="text-gray-400 text-lg md:text-xl">Unlock the full potential of AI with a plan that fits your needs.</p>
                </motion.div>
